    ...
    for (const c of [...src.caseStudies].reverse()) {
      await prisma.caseStudy.create({
        data: {
          ...
          beforeImageUrl: c.beforeImageUrl ?? (Array.isArray(c.beforeImages) ? c.beforeImages[0] ?? '' : ''),
          afterImageUrl: c.afterImageUrl ?? (Array.isArray(c.afterImages) ? c.afterImages[0] ?? '' : ''),
          beforeImages: toJsonList(c.beforeImages ?? (c.beforeImageUrl ? [c.beforeImageUrl] : [])),
          afterImages: toJsonList(c.afterImages ?? (c.afterImageUrl ? [c.afterImageUrl] : [])),
